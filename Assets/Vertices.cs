	...
	void Start () {
		...
		for (int i = 0; i < numberOfTriangles; ++i) {
			...
			if (edgePoints.Count == 2) {
				for (int j = 1; j < MAX_TRAFFIC_LIGHT; j++) {
					if (edgePoints[0] == trafficLights[j]) {
						Vector3 position;
						Vector3 rotation;
						if(Vector3.Distance(pathPlane.transform.position,edgePoints[0]) < Vector3.Distance(pathPlane.transform.position,edgePoints[1])){
							position = edgePoints[0];
							rotation = edgePoints[1] - edgePoints[0];
						} else {
							position = edgePoints[1];
